...
The document map is [`rules.md` §13](./rules.md#13-related). The binding execution
order — blocker-first, in four stages — is [`rules.md` §10](./rules.md#10-programme-goals).
...
Superseded by [`rules.md` §10](./rules.md#10-programme-goals), which reorders the same
...
it is in [`rules.md` §10](./rules.md#10-programme-goals); the standard behind that number is
...
Programme completion is defined in [`rules.md` §12](./rules.md#12-definition-of-done). These
...
Owned by [`rules.md` §1](./rules.md#1-rules-of-engagement). Two rules matter enough to repeat
...
Evidence stored under the baseline directory in [`rules.md` §11](./rules.md#11-evidence-contract). It raised two findings, both
now tracked in `Failures.md`: the absent `0004` Task 7 modules (blocker **B3**) and the
stale Product Studio list-route snapshots.

**Files:**
- Verify: `package.json`
- Record: [`rules.md` §10](./rules.md#10-programme-goals)
...
- [ ] **Step 3: Capture browser baseline**
...
- [ ] **Step 4: Record exact versions**

Run:

```powershell
pnpm why dockview-react fabric zustand `
  react-hook-form react-aria-components `
  @mastra/core @lancedb/lancedb @orama/orama
```

Expected: one intentional application version per direct dependency.
