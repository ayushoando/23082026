...
| Role | Ref | Owns | Env |
|------|-----|------|-----|
| **Products** | `erpweaiypimorcunaimz` | Marketing catalog, configurator | `PRODUCTS_DATABASE_URL` |
...
Refresh snapshot: `pnpm run r2:catalog-snapshot`. Degraded responses must say they are stale. Unverifiable state fails visibly.
...
Supabase / R2 / GitHub secrets / Vercel env live in dashboards. Blockers → `Failures.md`.
