...
Also: [`database/schema.md`](../database/schema.md) · [`database/drizzle.md`](../database/drizzle.md) · [`database/ops.md`](../database/ops.md).
...
| **CRM / Ops** | Admin CRM **demo** (localStorage) + `customer_queries` ops — real inbox is Admin DB | Top-level `/crm` product app |
...
| Edge & Security | **`site/proxy.ts` present** (Next 16); static headers in `site/next.config.js` (`/:path*`); there is no `site/next.config.mjs`; effective CSP is per-request in `proxy.ts`; secure cookies in `site/lib/security/cookies.ts`; input sanitization in `site/lib/security/sanitize.ts` |
