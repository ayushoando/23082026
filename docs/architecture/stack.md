...
`site/focss/base/scan.css` starts with `@import "tailwindcss"` (site, admin, studio).
Planner `entry.css` has its **own** `@import "tailwindcss"` and does not import `scan.css`.
The PostCSS pipeline runs `@tailwindcss/postcss`.
...
| State | `zustand` 5.0.15 | 7 files | Per-app canvas and workspace stores |
...
| Analytics & Telemetry | `@vercel/analytics`, `@vercel/speed-insights`, `@vercel/otel`, `@opentelemetry/api`, `newrelic`; GA4, New Relic Browser SPA loader, opt-in Node APM bridge, OTLP export, AI advisor spans, and `/api/metrics` | Browser loader is vendored and served through `/newrelic.js`; Node APM is hybrid/opt-in with duplicate HTTP/Next/undici instrumentation disabled. See [`../../OBSERVABILITY.md`](../../OBSERVABILITY.md) |
...
role. `polygon-clipping` has neither — retire it only when a current task lists the
removal (governance E4: git history is the archive for anything git tracks).

Do **not** reintroduce svgcanvas, Excalidraw, tldraw, Klecks, or FlexLayout as the
workspace dock unless a current plan task names that dock.
