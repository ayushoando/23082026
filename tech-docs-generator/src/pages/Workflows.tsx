...
const devWorkflow = [
  ...
  { step: '7', title: 'Commit', cmd: 'git commit -m "feat: ..."', detail: 'Commit when the current task requires it; secretlint via lint:secrets / gate.' },
  { step: '8', title: 'Push + PR', cmd: 'git push -u origin feature/<name>', detail: 'Create PR on GitHub when the current task requires it. Vercel preview if configured.' },
]
...
const commonTasks = [
  ...
  { icon: Rocket, title: 'Deploy to production', steps: ['Ensure focused checks + gate:fast green locally', 'Create PR + get approval', 'Run pnpm run release:gate', 'Run pnpm run ops vercel:prod per OPERATIONS_RUNBOOK.md'] },
]
