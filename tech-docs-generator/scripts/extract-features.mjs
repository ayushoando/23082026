  ...
  try {
    ...
    try {
      const resolved = require.resolve('typescript', {
        paths: [defaultRepoRoot, path.join(defaultRepoRoot, 'site'), path.join(defaultRepoRoot, 'tech-docs-generator')],
      })
...
function parsePlannerFeaturePages(filePath, repoRoot) {
  ...
  if (ts && ts.createSourceFile && ts.ScriptTarget) {
    try {
      const sourceFile = ts.createSourceFile(filePath, sourceText, ts.ScriptTarget.Latest ?? 99, true, ts.ScriptKind?.TS)
      const records = []
  ...
  // Regex fallback when full TypeScript AST is unavailable
  const records = []
  const blockRegex = /{\s*slug:\s*["']([^"']+)["'][\s\S]*?title:\s*["']([^"']+)["'][\s\S]*?tagline:\s*["']([^"']+)["'](?:[\s\S]*?icon:\s*([A-Za-z0-9_]+))?(?:[\s\S]*?summary:\s*["']([^"']+)["'])?(?:[\s\S]*?relatedSlugs:\s*\[([^\]]*)\])?[\s\S]*?}/g
  let match
  let index = 0
  while ((match = blockRegex.exec(sourceText)) !== null) {
    const [, slug, title, tagline, icon, summary, relatedRaw] = match
    const relatedSlugs = relatedRaw
      ? relatedRaw.split(',').map((s) => s.trim().replace(/^["']|["']$/g, '')).filter(Boolean)
      : []
    records.push(
      compactRecord({
        kind: 'planner-marketing',
        ...
        sourceKind: 'typed-feature-metadata',
        sourcePointer: `PLANNER_FEATURE_PAGES[${index}]`,
        slug: slug ?? `planner-feature-${index}`,
        title: title ?? slug ?? `Planner feature ${index}`,
        tagline: tagline ?? 'Planner marketing feature page',
        icon: icon ?? undefined,
...
function featurePageCandidates(repoRoot) {
  return [
    path.join(repoRoot, 'site', 'features', 'site', 'planner', 'landing', 'plannerFeaturePages.ts'),
