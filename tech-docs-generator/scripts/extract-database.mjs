...
export function extractDatabaseRecords({ repoRoot = defaultRepoRoot } = {}) {
  ...
  const migrationDirs = [
    path.join(repoRoot, 'site', 'platform', 'drizzle', 'migrations'),
    path.join(repoRoot, 'site', 'platform', 'supabase', 'migrations'),
    path.join(repoRoot, 'site', 'platform', 'supabase', 'migrations.admin'),
  ].filter((dir) => {
    try {
  ...
  const migrations = migrationDirs
    .flatMap((dir) => collectMigrations(dir, repoRoot))
    .sort((left, right) => left.path.localeCompare(right.path))
