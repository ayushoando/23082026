...
| GATE-RECHECK-01 | P1 | Ship bar not re-observed after the 2026-09-02 vitest fixes | Last full `pnpm run gate` exited 1 at `pnpm run test` (22 failed / 4215 succeeded). Targeted re-run of those files later verified 126/126. Build, coverage, docs, governance, and `test:browser:gate` never ran on the fixed tree | Re-run `pnpm run gate` on current `main`; delete this row only if that command exits 0 |
