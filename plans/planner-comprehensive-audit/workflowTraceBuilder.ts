...
export const PLANNER_WORKFLOW_BLUEPRINTS: readonly PlannerWorkflowBlueprint[] = [
  ...
  },
  {
    ...
    reachabilityNote:
      "The save path is wired, but static source shows no revision compare-and-swap or explicit conflict result; runtime behavior remains unverified.",
    coverage: coverage(
    ...
    stages: [
      ...
      ["site/lib/Planner/projectsStore.supabase.ts", "The Supabase adapter upserts the owner-bound project; development bypass can use the guarded disk adapter."],
      ["site/components/Planner/Planner.tsx", "A successful response updates local identity and route; all failures currently map to a generic save-error toast."],
      ["site/components/Planner/Planner.tsx", "The user sees a saved confirmation or a recoverable failure while the current canvas remains present."],
    ],
  },
  {
    key: "project-delete",
  ...
  },
  {
    ...
    routePath: "/ooplanner/projects/[id]",
    reachability: "gap-detected",
    reachabilityNote:
      "No Planner-owned online/offline listener or explicit offline/reconnect state was found; network failures currently collapse into transient load or generic save errors.",
    coverage: coverage(
    ...
    stages: [
      ...
      ["site/components/Planner/plannerLoadState.ts", "There is no dedicated offline state; load network failures use transient-error."],
      ["site/lib/Planner/plannerApi.ts", "Fetch rejects on network loss and exposes no explicit reconnect coordinator."],
      ...
      ["site/components/Planner/Planner.tsx", "Load maps to retryable transient state and save maps to a generic error toast; no reconnect conflict check is present."],
      ["site/components/Planner/PlannerProjectLoadState.tsx", "The user can retry a failed load, but an explicit offline indicator and deterministic reconnect action remain a traced gap."],
    ],
  },
  {
    ...
    reachability: "gap-detected",
    reachabilityNote:
      "Project payloads and handlers do not expose expected revision, idempotency, or a 409 recovery branch; failures map to a generic save toast.",
    coverage: coverage(
    ...
    stages: [
      ...
      ["site/components/Planner/Planner.tsx", "Save serializes the current in-memory project without an expected revision."],
      ["site/lib/Planner/plannerFabricSerialize.ts", "The payload preserves canvas content but carries no concurrency identity."],
      ["site/lib/Planner/plannerApi.ts", "updateProject sends a PATCH without revision or idempotency metadata."],
      ["site/app/api/Planner/projects/[id]/route.ts", "PATCH loads then mutates the record without compare-and-swap conflict enforcement."],
      ["site/lib/Planner/projectsStore.ts", "writeProjectRecord forwards the write without a revision contract."],
      ["site/lib/Planner/projectsStore.supabase.ts", "The Supabase adapter upserts without an expected-revision predicate."],
      ["site/components/Planner/Planner.tsx", "Any server failure maps to one generic save-error toast rather than a typed conflict decision."],
      ["site/components/Planner/Planner.tsx", "The user retains the current canvas but receives no explicit reload/compare/retry conflict recovery action."],
    ],
  },
  {
    ...
    reachability: "present-but-unverified",
    reachabilityNote:
      "Starting a new plan always asks for confirmation, but no dirty-state or beforeunload/navigation guard was found for other destructive exits.",
    coverage: coverage(
    ...
    stages: [
      ...
      ["site/components/Planner/Planner.tsx", "No explicit dirty-state store differentiates saved and unsaved documents."],
      ...
      ["site/components/Planner/Planner.tsx", "The user remains on the current plan after cancel or sees a fresh draft after confirm; other navigation remains an evidence gap."],
    ],
  },
];
