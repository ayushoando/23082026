...
- **Owner reversal — 2026-09-02 (durable):** the client-showcase deletion above was reversed on owner instruction to complete `plans/client-showcase-tabs`. The showcase is re-implemented against the current canonical registry (not the old plan draft), with presentation in `site/focss/site/components/clients/clients-showcase.css` to satisfy the style-token ratchet (the plan's literal arbitrary-value classes would have raised it); the two documented deviations (registry shape; styling/logo paths) are recorded in `plans/client-showcase-tabs/tasks.md`.
