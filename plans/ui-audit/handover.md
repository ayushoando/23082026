...
- **Reversal — 2026-09-02:** owner instructed completing `plans/client-showcase-tabs`, reversing the deletion above. The showcase is re-implemented against the **current canonical client registry** (not the old plan draft); presentation moved to `site/focss/site/components/clients/clients-showcase.css` to satisfy the style-token ratchet (the plan's literal arbitrary-value classes would have raised it); `plans/client-showcase-tabs/tasks.md` carries the two documented deviations.
