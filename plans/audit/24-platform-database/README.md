...
- **Governance baseline:** `P4_migration_no_rollback` was baselined at 42, tightened to 8 on 2026-09-01; observed debt is 0 and the baseline was **re-recorded to 0** on 2026-09-01 (remediation session; `check-governance --update --confirm`). `S2_stray_report` was tightened 22→1 in the same re-record.
...
- **Governance number (resolved 2026-09-01)** — rollback debt eliminated (observed 0 via grep) and the baseline file now records `P4_migration_no_rollback: 0` (re-recorded 2026-09-01; see top-of-file note).
