...
describe("product static params", () => {
  ...
  describe("buildProductStaticParams", () => {
    it("merges fallback products and deduplicates by category and slug", async () => {
      ...
      expect(params.every((entry) => entry.category === entry.category.toLowerCase())).toBe(true);
      expect(params.every((entry) => entry.product === entry.product.toLowerCase())).toBe(true);
      expect(params.some((entry) => entry.category === "tables" && entry.product === "crest")).toBe(
        true,
      );
      expect(params.some((entry) => entry.product === "Crest")).toBe(false);
      expect(
        params.some((entry) => entry.product.toLowerCase() === "categories"),
      ).toBe(false);
    });
    ...
    it("prefers canonical oando slugs when duplicate source keys exist", async () => {
      ...
      expect(seatingTask).toEqual([{ category: "seating", product: "task-chair" }]);
      expect(seatingTask.some((entry) => entry.product === "oando-seating--task-chair")).toBe(
        false,
      );
