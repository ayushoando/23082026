...
test.describe("site navigation smoke", () => {
  test("homepage loads with hero and progress dots", async ({ page }) => {
    ...
    const secondDot = page.getByRole("button", { name: "Show project image 2 of 7" });
  ...
  test("homepage hero exposes products CTA and trusted-by glass proof", async ({
    ...
    await Promise.all([
      page.waitForURL(/\/trusted-by\/$/, { timeout: 15_000 }),
      glassProof.click(),
    ]);
  ...
  test("/products catalog loads with first category card visible", async ({ page }) => {
    ...
    const firstCategoryCard = page.locator("main a.products-category-tile[href^='/products/'], main a.home-collection-card[href^='/products/']").first();
