...
test.describe("admin smoke â€” unauthenticated gate", () => {
  ...
  test("admin SVG publish API rejects an anonymous request", async ({
    ...
    if (authBypassOn) {
      // DEV_AUTH_BYPASS injects admin and skips CSRF — create may succeed (201).
      // Still record honesty; production gate runs with bypass off.
      expect([201, 400, 403, 401, 422]).toContain(response.status());
      return;
    }

    // Rejected without a valid session/CSRF — 403 preferred; 422 is also a hard deny.
