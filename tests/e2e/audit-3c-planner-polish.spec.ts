...
async function gotoWorkspace(page: import("@playwright/test").Page, name: string) {
  // reloadSafe: hard-reload tests must not wipe IndexedDB via init-script clear.
  await enterGuestPlannerWorkspace(page, { projectName: name, reloadSafe: true });
...
test("3c fix #9 — PlannerProjectMenu is live, renames the project, and stays in sync with the topbar", async ({ page }) => {
  ...
  await expect
    .poll(async () => page.getByTestId("btn-save").isEnabled(), { timeout: 20_000 })
    .toBe(true);
  // Prefer route binding after save; fall back to last-project-id localStorage.
  const savedUrl = await expect
    .poll(async () => page.url(), { timeout: 20_000 })
    .toMatch(/\/ooplanner\/projects\/[^/?#]+/)
    .then(() => page.url())
    .catch(async () => {
      const id = await page.evaluate(() =>
        localStorage.getItem("ooplanner.last-project-id"),
      );
      if (!id) throw new Error("Save did not bind project URL or last-project-id");
      await page.goto(`/ooplanner/projects/${id}`, {
        waitUntil: "domcontentloaded",
        timeout: 60_000,
      });
      return page.url();
    });
  logClick(`save bound project url: ${savedUrl}`);
  await page.reload({ waitUntil: "domcontentloaded" });
  await expect(page.locator(CANVAS_STAGE)).toBeVisible({ timeout: 30_000 });
  await expect
    .poll(async () => page.getByTestId("project-name").inputValue(), { timeout: 20_000 })
    .toBe(marker);
...
async function switchStep(page: import("@playwright/test").Page, step: "draw" | "place" | "review") {
  const btn = page.getByTestId("planner-workflow-bar").locator(`.pw-step-bar__btn[data-step="${step}"]`);
  await btn.click({ force: true });
  await expect(page.getByTestId("planner-workflow-bar")).toHaveAttribute("data-current", step, {
    timeout: 15_000,
  });
}
