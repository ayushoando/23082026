...
describe("04c proxy verify checklist", () => {
  ...
  it("CSP present on canvas-heavy /ooplanner path", async () => {
    ...
    const csp = buildContentSecurityPolicy("/ooplanner", { nonce: "live-smoke" });
    expect(csp).toMatch(/default-src 'self'/);
    expect(csp).toMatch(/script-src/);
    expect(csp).toContain("'nonce-live-smoke'");
    ...
    expect(resCsp).toBeTruthy();
    expect(resCsp).toContain("default-src");
    expect(resCsp).toMatch(/script-src[^;]*'nonce-/);
    expect(resCsp).not.toMatch(/script-src[^;]*'unsafe-inline'/);
  });
