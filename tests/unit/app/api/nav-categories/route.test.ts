...
describe("app/api/nav-categories/route.ts", () => {
  ...
  it("returns 200 with empty groups/categories when catalog is empty", async () => {
    vi.mocked(getCatalog).mockResolvedValue([]);
    const res = await GET(new Request("http://localhost/api/nav-categories"));
    expect(res.status).toBe(200);
    const body = await res.json();
    ...
    expect(body.categories[0].count).toBe(0);
  });

  it("cooldown fallback: when catalog serves fallback CompatCategories, route returns 200", async () => {
    vi.mocked(getCatalog).mockResolvedValue([
      {
        id: "fallback-cat",
        name: "Fallback Cat",
        ...
        series: [
          {
            ...
            name: "Series",
            description: "desc",
            products: [{ id: "fp1", slug: "fallback-chair", name: "Fallback Chair" }],
          },
        ],
      },
    ] as never);
