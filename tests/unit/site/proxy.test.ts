...
describe('proxy.ts', () => {
  ...
  describe('buildContentSecurityPolicy', () => {
    ...
    it('omits unsafe-eval on marketing paths', () => {
      expect(buildContentSecurityPolicy('/contact', { nonce })).not.toContain("'unsafe-eval'");
    });
    ...
    it('allows unsafe-eval on marketing paths in development (React Refresh)', () => {
      ...
      try {
        expect(buildContentSecurityPolicy('/contact', { nonce })).toContain("'unsafe-eval'");
      } finally {
        setNodeEnv(prev);
      }
    });
    ...
    it('allows unsafe-eval only on canvas-heavy product shells', () => {
      ...
      expect(buildContentSecurityPolicy('/ooplanner/projects', { nonce })).toContain("'unsafe-eval'");
      expect(buildContentSecurityPolicy('/admin', { nonce })).not.toContain("'unsafe-eval'");
    });

    it('allows Vercel Analytics and Speed Insights (dev scripts on va.vercel-scripts.com)', () => {
      const csp = buildContentSecurityPolicy('/contact', { nonce });
    ...
    /**
     * PX-S09 / SEC-4 — production script-src must not permit 'unsafe-inline'
     * (governance P2). Same-origin chunks use 'self'; inline uses nonce.
     */
    it('uses nonce + self and omits script-src unsafe-inline (PX-S09)', () => {
      const csp = buildContentSecurityPolicy('/contact', { nonce: 'abc123' });
      ...
      // style-src may still use unsafe-inline (not in P2 scope).
      expect(csp).toMatch(/style-src[^;]*'unsafe-inline'/);
    });

    it('omits script-src unsafe-inline on canvas-heavy paths too', () => {
      const csp = buildContentSecurityPolicy('/ooplanner', { nonce: 'canvas-n' });
      const scriptSrc = csp.split(';').map((p) => p.trim()).find((p) => p.startsWith('script-src'));
      expect(scriptSrc).toContain("'nonce-canvas-n'");
      expect(scriptSrc).not.toContain("'unsafe-inline'");
  ...
  describe('proxy', () => {
    ...
    it('sets a per-request script nonce CSP without script-src unsafe-inline (PX-S09)', async () => {
  ...
  describe('member-only / guest helpers', () => {
    ...
    it('flags www host for apex canonical redirect', () => {
      expect(
        shouldRedirectWwwToApex('www.example.com', 'https://example.com'),
      ).toBe(true);
      expect(
        shouldRedirectWwwToApex('example.com', 'https://example.com'),
      ).toBe(false);
      expect(shouldRedirectWwwToApex('localhost:3000', 'http://localhost:3000')).toBe(
        false,
      );
    });
