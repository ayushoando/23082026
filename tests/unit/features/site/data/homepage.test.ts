...
describe("homepage data", () => {
  ...
  it("partnership content is One and Only only (no third-party partner brand)", () => {
    // Visible UI brand is "One and Only"; SEO/meta may still use One&Only elsewhere.
    expect(HOMEPAGE_PARTNERSHIP_CONTENT.title).toEqual([
      "One and Only",
      "Office Furniture",
    ]);
    expect(HOMEPAGE_PARTNERSHIP_CONTENT.image.src).toBe("/logo.webp");
    expect(HOMEPAGE_PARTNERSHIP_CONTENT.image.alt).toMatch(/One and Only/i);
