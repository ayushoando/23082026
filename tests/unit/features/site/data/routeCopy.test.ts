...
describe("route copy — careers", () => {
  it("career page lists openings and support routing", () => {
    expect(CAREER_PAGE_JOBS).toHaveLength(2);
...
describe("route copy — legal and catalog routes", () => {
  ...
  it("groups clients by descending count and uses sector name as a tie-break", () => {
    expect(groupClientsBySector([])).toEqual([]);
    expect(
      groupClientsBySector([
        { name: "A", sector: "Finance" },
        { name: "B", sector: "Energy" },
        { name: "C", sector: "Finance" },
        { name: "D", sector: "Automotive" },
        { name: "E", sector: "Energy" },
      ]),
    ).toEqual([
