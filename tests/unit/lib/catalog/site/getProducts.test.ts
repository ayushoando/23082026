...
describe("catalog/site/getProducts", () => {
  ...
  it("retries a live catalog tree when the cached tree is empty", async () => {
    buildCatalogLive
      .mockResolvedValueOnce([])
      .mockResolvedValueOnce([{ id: "cat-live", name: "Live" }]);

    const mod = await import("@/lib/catalog/site/getProducts");
    await expect(mod.getCatalog()).resolves.toEqual([{ id: "cat-live", name: "Live" }]);
    expect(buildCatalogLive).toHaveBeenCalledTimes(2);
  });
});
