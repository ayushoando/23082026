...
vi.mock("@/features/site/data/localCatalogIndex.json", () => ({
  default: [
    ...
    {
      id: "p3",
      slug: "legacy-name-only",
      category_id: "seating",
      name: "Name Only Chair",
      images: ["/assets/catalog/seating/name-only/image-1.jpg"],
    },
    {
      id: "p4",
      slug: "crox",
      category_id: "seating",
      name: "Crox",
      images: ["/assets/catalog/seating/crox/image-1.jpg"],
    },
    {
      id: "p5",
      slug: "missing-images-entry",
      category_id: "tables",
      name: "No Images Listed",
    }
  ]
}));
...
describe("Image Metadata Resolver", () => {
  ...
  describe("catalogSlugForProduct", () => {
    it("returns correct canonical slugs for categories", () => {
      ...
      expect(catalogSlugForProduct("oando-", "Nameless")).toBeNull();
    });
  });
  ...
  describe("resolveProductImages", () => {
    ...
    it("resolves from catalog index by exact name when slug and dir miss", () => {
      mockExistsSync.mockImplementation((filePath: string) => {
        return String(filePath).replace(/\\/g, "/").includes("name-only/image-1.jpg");
      });
      ...
      expect(result?.source).toBe("catalog-index-name");
      expect(result?.matchedSlug).toBe("legacy-name-only");
    });

    it("skips unpublished index slugs and empty image lists", () => {
      mockExistsSync.mockReturnValue(false);
      expect(resolveProductImages({ slug: "crox", name: "Crox" })).toBeNull();
      expect(
        resolveProductImages({ slug: "missing-images-entry", name: "No Images Listed" }),
      ).toBeNull();
    });
