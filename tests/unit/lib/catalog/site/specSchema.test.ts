...
describe("collectProductImages", () => {
  ...
  it("treats missing image arrays and empty variant galleries as empty", () => {
    const images = collectProductImages(
      baseProduct({
        images: undefined,
        sceneImages: undefined as unknown as string[],
        variants: [
          {
            id: "v-empty",
            variantName: "Bare",
            galleryImages: undefined as unknown as string[],
          },
        ],
      }),
    );
    expect(images).toContain("https://cdn.example.com/chair.webp");
  });
});
...
describe("auditCompatProduct", () => {
  ...
  it("falls back to specs materials and features when detailedInfo is empty", () => {
    ...
    const codes = new Set(issues.map((issue) => issue.code));
    expect(codes.has("missing_materials")).toBe(false);
    expect(codes.has("missing_features")).toBe(false);
  });
});
