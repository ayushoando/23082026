...
describe("marketingImages", () => {
  it("rejects line SVG and placeholder paths for marketing", () => {
    expect(isUsableMarketingImage("")).toBe(false);
    expect(isUsableMarketingImage("   ")).toBe(false);
  ...
  it("pickMarketingImage chooses first photo candidate", () => {
    ...
    expect(pickMarketingImage(null, undefined, "  ")).toBe("");
  });
  ...
  it("pickCategoryTileImage prefers /images/catalog/oando- photography", () => {
    expect(
      pickCategoryTileImage([
        "/assets/catalog/products/loose.webp",
        "/images/catalog/oando-seating--arvo/gallery/image-01.webp",
      ]),
    ).toBe("/images/catalog/oando-seating--arvo/gallery/image-01.webp");
  });

  it("pickCategoryTileImage returns empty when no candidates remain", () => {
    expect(pickCategoryTileImage([])).toBe("");
  });
