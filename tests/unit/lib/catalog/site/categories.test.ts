...
describe("site catalog categories", () => {
  ...
  describe("getCanonicalSubcategoryId and label", () => {
    ...
    it("defaults seating to fabric chairs and unknown category to General", () => {
      expect(resolveCanonicalSubcategory("seating", { productName: "Generic" })).toEqual({
        id: "fabric",
        label: "Fabric chairs",
      });
      expect(resolveCanonicalSubcategory("mystery", {})).toEqual({
        id: "general",
        label: "General",
      });
    });

    it("classifies subcategory from specs materials and features", () => {
      expect(
        classifyToRequestedSubcategory("seating", {
          product: makeProduct({
            name: "Task",
            specs: { materials: ["mesh"], features: ["tilt"] },
          }),
  ...
  describe("catalog navigation helpers", () => {
    it("returns labels, descriptions, and hrefs", () => {
      ...
      expect(getCatalogCategoryDescription("unknown", "Fallback copy")).toBe("Fallback copy");
      expect(getRequestedCategoryRouteSegment("oando-")).toBe("products");
  ...
  describe("buildRequestedCategoryCatalog", () => {
    it("returns no fake category shells for an empty catalog", () => {
      expect(buildRequestedCategoryCatalog([])).toEqual([]);
    });
    ...
    it("sorts non-seating series and products by name", () => {
      const rebuilt = buildRequestedCategoryCatalog([
        {
          id: "oando-tables",
          name: "Tables",
          description: "",
          series: [
            {
              ...
              description: "",
              products: [
                makeProduct({ id: "z-b", name: "Zeta B" }),
                makeProduct({ id: "z-a", name: "Zeta A" }),
              ],
            },
            {
              id: "alpha",
              name: "Alpha Series",
              description: "",
              products: [makeProduct({ id: "a-1", name: "Alpha Table" })],
            },
          ],
        },
      ]);
      const tables = rebuilt.find((category) => category.id === "tables");
      expect(tables?.series.map((series) => series.name)).toEqual([
        "Alpha Series",
        "Zeta Series",
      ]);
      expect(tables?.series[1]?.products.map((product) => product.name)).toEqual([
        "Zeta A",
        "Zeta B",
      ]);
    });
