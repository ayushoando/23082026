...
describe("site catalog filters", () => {
  ...
  describe("productMatchesSubcategoryFilters", () => {
    ...
    it("matches every product when no subcategory filters are set", () => {
      expect(
        productMatchesSubcategoryFilters("workstations", heightAdjustableProduct, []),
      ).toBe(true);
    });

    it("matches when only subcategory label is set on the product", () => {
      expect(
        productMatchesSubcategoryFilters(
          "workstations",
