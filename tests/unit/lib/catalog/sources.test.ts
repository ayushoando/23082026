...
describe("sources", () => {
  ...
  describe("fetchProductsByCategoryLive", () => {
    ...
    it("should return filtered fallback when Drizzle returns null while configured", async () => {
      vi.mocked(canQueryCatalogDatabase).mockReturnValue(true);
      vi.mocked(fetchCatalogProductsByCategoryLive).mockResolvedValue(null);

      const res = await fetchProductsByCategoryLive("desks");
      expect(res).toEqual([mockFallbackProducts[1]]);
    });

    it("should return filtered fallback when the category query throws", async () => {
      vi.mocked(canQueryCatalogDatabase).mockReturnValue(true);
      vi.mocked(fetchCatalogProductsByCategoryLive).mockRejectedValue(
        new Error("db down"),
      );
  ...
  describe("fetchProductByUrlKeyLive", () => {
    ...
    it("should fall back to the slug lookup when Drizzle returns null while configured", async () => {
      vi.mocked(canQueryCatalogDatabase).mockReturnValue(true);
      vi.mocked(fetchCatalogProductBySlugLive).mockResolvedValue(null);

      const res = await fetchProductByUrlKeyLive("desk-b");
      expect(res).toEqual(mockFallbackProducts[1]);
    });

    it("should return null when a missing slug is not present in the fallback", async () => {
      vi.mocked(canQueryCatalogDatabase).mockReturnValue(true);
      vi.mocked(fetchCatalogProductBySlugLive).mockResolvedValue(null);

      const res = await fetchProductByUrlKeyLive("does-not-exist");
      expect(res).toBeNull();
    });
  });
