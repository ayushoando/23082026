...
describe("fetchAdminConfiguratorCatalog", () => {
  ...
  it("omits category and active predicates when no filters are supplied", async () => {
    isProductsDatabaseConfigured.mockReturnValue(true);
    orderBy.mockResolvedValue([]);
    ...
    expect(rows).toEqual([]);
    // With no filters, both ternaries resolve to `undefined`, so the drizzle
    // `and(...)` receives only undefined predicates.
    expect(where).toHaveBeenCalledTimes(1);
    const predicate = where.mock.calls[0]?.[0] as unknown[] | undefined;
    expect(predicate).toEqual([undefined, undefined]);
  });

  it("keeps the active=false predicate distinct from an unset filter", async () => {
    isProductsDatabaseConfigured.mockReturnValue(true);
    orderBy.mockResolvedValue([]);
    ...
    await fetchAdminConfiguratorCatalog({ active: false });

    const rawPredicate = where.mock.calls[0]?.[0] as unknown;
    const predicate = (Array.isArray(rawPredicate) ? rawPredicate : []) as unknown[];
    // category unset -> undefined; active===false -> an eq(...) predicate object.
    expect(predicate[0]).toBeUndefined();
    expect(predicate[1]).toEqual({ a: "active", b: false });
  });
});
