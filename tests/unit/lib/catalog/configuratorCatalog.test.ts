...
describe("configurator catalog mapping", () => {
  ...
  it("falls back to id and nulls when optional product fields are absent", () => {
    const minimal: Product = {
      ...
      sizingType: "discrete",
      sizeOptions: [
        { sku: "PED-3", label: "3-Drawer", dim: { L: 400, D: 500 } },
      ],
      specs: { dimensions: "", materials: [], features: [] },
      series_id: "storage-storage",
      series_name: "Storage",
      created_at: "2024-01-01",
    };

    const row = productToRow(minimal);

    expect(row.slug).toBe("bare-product");
    expect(row.family).toBeNull();
    expect(row.brand_name).toBeNull();
    expect(row.workstation).toBeNull();
    expect(row.default_footprint).toBeNull();
    expect(row.derived_rules).toBeNull();
    expect(row.thumbnail_url).toBeNull();
    expect(row.model_3d_url).toBeNull();
    expect(row.description).toBeNull();
    expect(row.size_options).toEqual(minimal.sizeOptions);
  ...
  it("preserves populated size options and defaults nullish materials to empty", () => {
    const sizeOptions: ConfiguratorProductRow["size_options"] = [
      { sku: "PED-2", label: "2-Drawer", dim: { L: 400, D: 500 } },
    ];
    ...
    const product = rowToProduct({
      ...
      default_footprint: null,
      derived_rules: null,
      // materials is non-null in the row type, but the runtime `?? []` guards
      // against nullish payloads coming back from the database.
      materials: null as unknown as string[],
      thumbnail_url: null,
      model_3d_url: null,
      description: null,
    });

    expect(product.sizeOptions).toEqual(sizeOptions);
    expect(product.specs.materials).toEqual([]);
