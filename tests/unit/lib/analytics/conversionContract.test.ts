...
describe("conversionContract", () => {
  ...
  it("emits project_start and first_placement after consent", () => {
    trackPlannerProjectStart("proj-1", "starter-template");
    trackPlannerFirstPlacement("proj-1", "desk-1");
    expect(mockTrack).toHaveBeenCalledWith(CONVERSION_EVENTS.PROJECT_START, {
      projectId: "proj-1",
      source: "starter-template",
      locale: "en",
    });
    expect(mockTrack).toHaveBeenCalledWith(CONVERSION_EVENTS.FIRST_PLACEMENT, {
      projectId: "proj-1",
      productId: "desk-1",
      locale: "en",
    });
