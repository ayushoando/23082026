...
describe("r2Catalog", () => {
  ...
  it("rewrites seating/non-leather to the on-disk subcategory folder", async () => {
    const { catalogAssetR2Keys } = await import("@/lib/storage/r2Catalog");
    const keys = catalogAssetR2Keys(
      "/assets/catalog/seating/non-leather/oando-seating--arvo/gallery/image-01.webp",
    );
    expect(keys).toContain(
      "catalog/seating/fabric/oando-seating--arvo/image-1.webp",
    );
    expect(keys).toContain(
      "catalog/seating/fabric/oando-seating--arvo/image-01.webp",
    );
  });
  ...
  it("listR2ObjectKeys returns [] without credentials", async () => {
    const mod = await import("@/lib/storage/r2Catalog");
    mod.resetR2ListCache();
    await expect(mod.listR2ObjectKeys("marketing/clients/Titan/")).resolves.toEqual([]);
  });

  it("listR2ObjectKeys lists keys under a prefix", async () => {
    process.env.CLOUDFLARE_S3_URL = "https://example.r2.cloudflarestorage.com";
    process.env.CLOUDFLARE_R2_ACCESS_KEY_ID = "key";
    process.env.CLOUDFLARE_R2_SECRET_ACCESS_KEY = "secret";
    send.mockResolvedValue({
      Contents: [
        { Key: "marketing/clients/Titan/" },
        { Key: "marketing/clients/Titan/titan-hero.webp" },
      ],
      IsTruncated: false,
    });
    ...
    const mod = await import("@/lib/storage/r2Catalog");
    mod.resetR2ListCache();
    const keys = await mod.listR2ObjectKeys("marketing/clients/Titan/");
    expect(keys).toEqual(["marketing/clients/Titan/titan-hero.webp"]);
    expect(ListObjectsV2Command).toHaveBeenCalled();
  });
});
