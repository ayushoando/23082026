...
describe("validate-launch-env (name-mirror)", () => {
  ...
  it("skips strict launch validation in CI when repository secrets are absent", async () => {
    const { validateWorkstationEnv } = await import(
      "../../../scripts/general/workstation-env.mjs"
    );
    const env = { CI: "true" };
    expect(isCiGateWithoutSecrets(env)).toBe(true);
    const launch = validateLaunchEnv(env);
    expect(launch.ok).toBe(true);
    expect(launch.skipped).toBe(true);
    const workstation = validateWorkstationEnv(env);
    expect(workstation.ok).toBe(true);
    expect(workstation.skipped).toBe(true);
  });
