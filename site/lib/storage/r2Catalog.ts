...
const SEATING_LEATHER = new Set(["grace", "pinnacle", "moonlight", "rider"]);
const SEATING_CAFE = new Set([
  "cafe-sleek",
  "caneva",
  "caneva-high",
  "casca",
...
function seatingSubcategory(skuFolder: string): string {
  ...
  const slug = (m?.[1] || skuFolder).toLowerCase();
  if (SEATING_LEATHER.has(slug)) return "leather";
  if (SEATING_CAFE.has(slug)) return "cafe";
  if (SEATING_FABRIC.has(slug)) return "fabric";
  return "mesh";
}

/** DB leftover `seating/non-leather/SKU` → on-disk `seating/{cafe|fabric|leather|mesh}/SKU`. */
export function rewriteSeatingCatalogFolder(webPath: string): string {
  return webPath.replace(
    /(^|\/)seating\/non-leather\/(oando-seating--[^/]+)/i,
    (_full, lead: string, sku: string) =>
      `${lead}seating/${seatingSubcategory(sku)}/${sku}`,
  );
}
...
export function catalogAssetR2Keys(webPath: string): string[] {
  const trimmed = rewriteSeatingCatalogFolder(
    webPath.trim().replace(/^\/+/, ""),
  );
  if (!trimmed) {
    return [];
  }
  ...
  if (trimmed.includes("/gallery/")) {
    variants.add(trimmed.replace(/\/gallery\//g, "/"));
  }

  for (const variant of variants) {
    keys.add(variant);
    if (variant.startsWith("assets/catalog/")) {
      keys.add(variant.slice("assets/".length));
    }
    ...
    if (numbered) {
      ...
      keys.add(unpadded);
      keys.add(padded);
      if (unpadded.startsWith("assets/catalog/")) {
        keys.add(unpadded.slice("assets/".length));
        keys.add(padded.slice("assets/".length));
      }
    }
  }
...
const R2_LIST_TTL_MS = 60_000;
const r2ListCache = new Map<string, { at: number; keys: string[] }>();

export function resetR2ListCache(): void {
  r2ListCache.clear();
}

/** List object keys under a prefix. Empty when R2 is not configured. */
export async function listR2ObjectKeys(prefix: string): Promise<string[]> {
  const normalized = prefix.replace(/^\/+/, "");
  if (!normalized || !resolveR2Endpoint() || !resolveR2Credentials()) {
    return [];
  }

  const cached = r2ListCache.get(normalized);
  if (cached && Date.now() - cached.at < R2_LIST_TTL_MS) {
    return cached.keys;
  }
  ...
  try {
    ...
    r2ListCache.set(normalized, { at: Date.now(), keys });
    return keys;
  } catch {
    return [];
  }
...
export async function readR2ObjectText(
  key: string,
  bucket = resolveCatalogBucketName(),
): Promise<string | null> {
  if (!resolveR2Credentials()) {
    return null;
  }
...
export async function probeR2CatalogAccess(options?: {
  ...
  if (
    !force &&
    r2ProbeCache &&
    Date.now() - r2ProbeCache.at < R2_PROBE_TTL_MS
  ) {
    return {
      ok: r2ProbeCache.ok,
