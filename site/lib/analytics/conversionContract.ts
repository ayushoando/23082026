 ...
 * Planner first-run: PROJECT_START / FIRST_PLACEMENT (Plan 04a).
 */
export function trackPlannerProjectStart(
  projectId: string,
  source: string,
  locale = "en",
): void {
  trackConversionEvent(CONVERSION_EVENTS.PROJECT_START, { projectId, source, locale });
}

export function trackPlannerFirstPlacement(
  projectId: string,
  productId: string,
  locale = "en",
): void {
  trackConversionEvent(CONVERSION_EVENTS.FIRST_PLACEMENT, { projectId, productId, locale });
}
