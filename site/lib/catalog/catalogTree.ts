...
export async function buildCatalogLive(): Promise<CompatCategory[]> {
  ...
  if (hasRecentFailure) {
    const fallbackProducts = await resolveCatalogFallbackProducts();
    if (fallbackProducts.length > 0) {
      products = fallbackProducts;
      const categoryIds = [
        ...new Set(fallbackProducts.map((product) => product.category_id).filter(Boolean)),
      ];
...
export function resetCatalogFailureCooldownForTests(): void {
  lastCatalogFailureAt = 0;
}
