  ...
  if (!navCategoriesPromise) {
    navCategoriesPromise = fetch("/api/nav-categories/")
      .then((r) => {
        ...
        return r.json() as Promise<NavCategoriesPayload>;
      })
      .catch(() => {
...
const siteHeaderBaseClass =
  "site-header fixed top-0 left-0 z-50 w-full border-b border-soft transition-shadow [background-color:var(--surface-glass-strong)] [transition-duration:var(--motion-fast)] [transition-timing-function:var(--ease-standard)]";
