...
import { MOTION_EASE, MOTION_TOKENS, useHydrationStableReducedMotion } from "@/lib/helpers/motion";
...
const fadeIn: Variants = {
  ...
  visible: (delay: number) => ({
    opacity: 1,
    transition: { duration: MOTION_TOKENS.medium, delay, ease: MOTION_EASE },
  }),
};
...
export function PlannerHeroDemo() {
  const reduced = useHydrationStableReducedMotion();
  ...
  const motionInitial = reduced === true ? false : ("hidden" as const);
  const svgKey = reduced === true ? "static" : `cycle-${cycle}`;
