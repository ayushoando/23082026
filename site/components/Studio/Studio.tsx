...
import { StudioAiPanel } from "@studio/components/dock/StudioDockPanels";
import {
  asOo,
  getErrorMessage,
  getApiErrorMessage,
...
const Studio = () => {
  ...
  // Start collapsed so first paint matches settled shell (WRK-S15 CLS ~0.30 was
  // left dock open 300px then collapse → tool-rail/canvas jump). Dock-tab buttons
  // and focusDockPanel reopen panels; onEmpty keeps them collapsed.
  const [leftCollapsed, setLeftCollapsed] = useState(true);
  const [rightCollapsed, setRightCollapsed] = useState(true);
  useEffect(() => {
    // Keep collapsed below --breakpoint-sm (640px); do not auto-expand on wide
    // (that would reintroduce first-paint CLS). User opens via dock tabs.
  ...
  }, [autoFit, ready, core, core.fitToContent]);
