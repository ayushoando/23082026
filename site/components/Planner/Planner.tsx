...
import { PlannerBoqPanel, PlannerColorPanel, PlannerValidationPanel } from "@planner/components/dock/PlannerDockPanels";
import {
  asOo,
  errMessage,
  tag,
  DEFAULT_SHEET,
  PLANNER_LAST_PROJECT_KEY,
  PLANNER_TOOLS_PANELS,
  PLANNER_LEFT_CATALOG_PANELS,
  PLANNER_DRAW_SHEET_PANEL,
  PLANNER_DRAW_COLOR_PANEL,
  PLANNER_RIGHT_PLACE_PANELS,
  PLANNER_RIGHT_REVIEW_BASE,
  type PlannerTool,
  type Point2D,
...
import {
  trackPlannerFirstPlacement,
  trackPlannerProjectStart,
...
const Planner = ({
  ...
  const toolsPanel = usePanelResize({
    storageKey: "planner.panel.tools.w",
    defaultWidth: 168,
    minWidth: 88,
    maxWidth: 280,
    edge: "start",
  ...
  }, [leftPanel.width, rightPanel.width, toolsPanel.width, leftCollapsed, rightCollapsed, toolsCollapsed]);
  ...
  }, [autoFit, ready, core, core.fitToContent, sheet.width_mm, sheet.height_mm]);
  ...
  useEffect(() => {
    ...
    const onDown = (opt: TPointerEventInfo<TPointerEvent>) => {
      ...
      if (tool === "wall") {
        ...
        if (!dimStartRef.current) {
          ...
          const origin = dimStartRef.current;
          const dx = p.x - origin.x;
          const dy = p.y - origin.y;
          const distMm = Math.round(Math.sqrt(dx * dx + dy * dy) / SCALE_PX_PER_MM);
          const mid = { x: (p.x + origin.x) / 2, y: (p.y + origin.y) / 2 };
          const line = new fabric.Line([origin.x, origin.y, p.x, p.y], { stroke: OO.obb600, strokeWidth: 1, strokeUniform: true, selectable: false, evented: false });
          ...
          c.add(g);
          g.setCoords();
          c.fire("object:modified", { target: g });
          c.requestRenderAll();
          dimStartRef.current = null;
        }
    ...
    const onMove = (opt: TPointerEventInfo<TPointerEvent>) => {
      ...
      } else if (tool === "dimension" && dimStartRef.current && drawing instanceof fabric.Line) {
        drawing.set({ x2: p.x, y2: p.y });
      }
      c.requestRenderAll();
    };
  ...
  }, [ready, tool, fabricRef, snapEnabled, gridSize, history, history.suspend, history.resume, flag]);
  ...
  const placeFurnitureAt = useCallback(async (item: FurnitureItem, abs: { x: number; y: number }) => {
    ...
    try {
      if (url) {
        ...
        if (!firstPlacementRef.current) {
          firstPlacementRef.current = true;
          trackPlannerFirstPlacement(projectIdRef.current ?? "guest-draft", item.id);
        }
        return;
      }
    } catch {
  ...
  const plannerCtx = {
    ...
    placeFurnitureItem,
    tool,
    setTool: (next: string) => setTool(next as PlannerTool),
    undo: history.undo,
    redo: history.redo,
  };
  ...
  return (
    ...
    >
      ...
      <aside
        className="side-panel side-panel--tools side-panel--dock"
        data-testid="tools-dock"
        aria-label="Drawing tools"
        data-collapsed={toolsCollapsed}
        style={{
          width: toolsCollapsed ? 0 : toolsPanel.width,
          display: toolsCollapsed ? "none" : undefined,
        }}
      >
        {!toolsCollapsed ? (
          <DockShell
            panels={PLANNER_TOOLS_PANELS}
            storageKey="planner.dock.tools.v1"
          />
        ) : null}
      ...
      <div className="canvas-stage" data-testid="canvas-stage" data-rulers="true">
        ...
        >
          ...
          <DockPanelButtons
            items={[{ id: "tools", label: "Tools", testId: "dock-tab-tools", icon: "cursor" }]}
            activeId={toolsCollapsed ? null : "tools"}
            onSelect={() => setToolsCollapsed((collapsed) => !collapsed)}
          />
          <div className="overlay-sep" />
          {plannerStep === "draw" ? (
            <>
              <div className="overlay-toggle-group" role="group" aria-label="Draw panels">
                <button
                  ...
                  title="Sheet"
                >
                  <PhIcon name="rect" size={18} />
                  <span className="overlay-btn-label">Sheet</span>
                </button>
                <button
                  type="button"
                  className="overlay-toggle-group__btn"
                  ...
                  title="Color"
                >
                  <PhIcon name="brush" size={18} />
                  <span className="overlay-btn-label">Color</span>
                </button>
              </div>
              <div className="overlay-sep" />
            </>
          ...
          {plannerStep === "place" ? (
            <>
              <DockPanelButtons
                items={[
                  { id: "catalog", label: "Inventory", testId: "dock-tab-catalog", icon: "layers" },
                ]}
              ...
              <DockPanelButtons
                items={[
                  { id: "props", label: "Properties", testId: "dock-tab-props", icon: "properties" },
                ]}
                activeId={rightCollapsed ? null : activeRightDock}
                onSelect={(id) => focusDockPanel("right", id)}
              />
              <div className="overlay-sep" />
              <button className="btn btn--sm" type="button" onClick={() => setAutoOpen(true)} data-testid="btn-auto-arrange" title="Auto-arrange" aria-label="Auto-arrange">
                <PhIcon name="grid" size={18} />
                <span className="overlay-btn-label">Auto-arrange</span>
              </button>
              <div className="overlay-sep" />
            </>
          ) : null}
          {plannerStep === "review" ? (
            <>
              <DockPanelButtons
                items={[
                  { id: "boq", label: "BOQ", testId: "dock-tab-boq", icon: "download" },
                  { id: "sheet", label: "Sheet", testId: "dock-tab-sheet", icon: "rect" },
                  { id: "layers", label: "Layers", testId: "dock-tab-layers", icon: "layers" },
                  { id: "color", label: "Color", testId: "dock-tab-color", icon: "brush" },
                  { id: "props", label: "Properties", testId: "dock-tab-props", icon: "properties" },
                ]}
                activeId={rightCollapsed ? null : activeRightDock}
                onSelect={(id) => focusDockPanel("right", id)}
          ...
          >
            <PhIcon name="star" size={18} />
            <span className="overlay-btn-label">AI</span>
          </button>
          <div className="overlay-sep" />
          <PlannerUnitPill />
          <div className="overlay-sep" />
