...
export function OneAndOnlyLogo({ className, variant = "orange" }: LogoProps) {
  ...
  const alt =
    variant === "mark" ? "One and Only" : "One and Only Furniture";

  // Wordmark: keep intrinsic aspect on the box so CSS height does not leave
  // width:0 until paint (Studio topbar CLS WRK-S15). Mark is square.
  const aspectStyle =
    variant === "mark"
      ? undefined
