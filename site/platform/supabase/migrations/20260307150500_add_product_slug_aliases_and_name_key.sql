...
-- rollback
-- delete from public.product_slug_aliases
--   where alias_slug = 'fluid-x'
--     and reason = 'canonicalize_mesh_slug_duplicate';
-- drop view if exists public.product_name_collisions;
-- drop index if exists idx_products_category_normalized_name_key;
-- drop trigger if exists trg_set_products_normalized_name_key on public.products;
-- drop function if exists public.set_products_normalized_name_key();
-- drop function if exists public.compute_normalized_product_name_key(text);
-- alter table public.products drop column if exists normalized_name_key;
-- drop policy if exists "Allow public read access on product_slug_aliases" on public.product_slug_aliases;
-- drop trigger if exists trg_touch_product_slug_aliases_updated_at on public.product_slug_aliases;
-- drop function if exists public.touch_product_slug_aliases_updated_at();
-- drop table if exists public.product_slug_aliases;
-- pgcrypto left in place
