...
-- rollback
-- drop index if exists public.idx_product_slug_aliases_canonical;
-- this file may have created public.product_slug_aliases as a table (legacy
-- columns) or as a view over catalog_product_slug_aliases. Drop only the
-- object this file added; do not drop catalog_product_slug_aliases.
-- drop view if exists public.product_slug_aliases;
-- drop table if exists public.product_slug_aliases;
-- is_active column: only drop if this file added it to a pre-existing table
-- (later schema may still need it — inspect before running):
-- alter table public.product_slug_aliases drop column if exists is_active;
