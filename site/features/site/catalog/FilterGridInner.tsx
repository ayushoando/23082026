...
export function AdvancedFilterGridInner({
  ...
  const data: FilterResponse | undefined = filterQueryData;

  const clientFilteredFallback = useMemo(
    () =>
      applyCatalogProductFilters(
        categoryId,
        fallbackProducts,
        activeFiltersToCatalogApplied(effectiveFilters),
      ),
    [categoryId, effectiveFilters, fallbackProducts],
  );
  ...
  const isInitialFilteredLoad =
    hasFilterQuery && isPending && data?.products === undefined && !error;
