...
import { derivePublicProductUrlKey } from "@/lib/catalog/productUrlKey";
...
function publicProductUrlKey(row: CategoryResolutionRow, fallback: string): string {
  return derivePublicProductUrlKey(row) || fallback;
}
...
export async function generateMetadata({
  ...
  const canonicalProductUrlKey = publicProductUrlKey(
    product as CategoryResolutionRow,
    productResolution.canonicalSlug || productUrlKey,
  );
...
async function ProductContent({
  ...
  const requestedUrlKey = productUrlKey.trim().toLowerCase();
  const canonicalUrlKey = canonicalProductUrlKey.trim().toLowerCase();
  if (
    resolvedCategoryId !== normalizedUrlCategory ||
    (canonicalUrlKey && requestedUrlKey !== canonicalUrlKey)
  ) {
