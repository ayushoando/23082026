...
description: Coordinates and executes approved Kiro spec tasks, changes only owned files, reconciles worker handoffs, and uses approved non-test checks.
resources:
  - file://AGENTS.md
  - file://START.md
  - file://README.md
...
4. For the Kiro configuration rewrite, own coordinator/integration Tasks 7–10 and only the paths assigned to Agent A in `tasks.md`. Make worker-path integration fixes only after all affected workers stop and exclusive ownership is confirmed.
