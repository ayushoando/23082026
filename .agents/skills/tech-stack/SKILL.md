...
│   │   └── security/         # Input sanitization, CSP helpers
...
- **Style Token Ratchet:** Verified by `pnpm run check:style-tokens` (must remain at or below baseline debt cap).
...
- Multi-provider fallback chain: Google Gemini → Anthropic Claude → OpenAI GPT.
...
- Verified via `pnpm run check:i18n:parity` and `pnpm run check:site-ui`.

---

## 11. Security Hardening

- **CSP:** Content Security Policy with per-request cryptographic nonces in [`site/proxy.ts`](file:///d:/23082026/site/proxy.ts) and static security headers in [`site/next.config.js`](file:///d:/23082026/site/next.config.js).
- **Input Sanitization:** Centralized in [`site/lib/security/`](file:///d:/23082026/site/lib/security/).
- **Secure Cookies:** HttpOnly, Secure, SameSite=Strict for auth and session tokens.
- **Secret Scanning:** `pnpm run scan:secrets` verifies zero credential exposure in tracked files.
- **Service Role Isolation:** `SUPABASE_ADMIN_SERVICE_ROLE_KEY` is never exposed to `NEXT_PUBLIC_` client variables via `assertNotServiceRoleKey`.

---

## 12. Testing Subsystem & Verification Lanes
...
1. `pnpm run test:audit` — Runs anti-hollow, gate-skip, and eslint-disable audits.
2. `pnpm run test:audit:fake-test` — Rejects mocked units under test.
3. Gate skips audit rejects unauthorized `.skip` and `.only`.
4. ESLint disable audit enforces 5-file cap on `react-hooks/exhaustive-deps`.
5. API route safety audit validates route handlers and auth barriers.

### Playwright E2E Testing
- Origin must be `http://localhost:3000` (never `127.0.0.1`).
- Verified against standard viewports: Desktop (1280×800) and Mobile (390×844).

---

## 13. Release Gating & Quality Floors
...
## 14. Deployment, Vercel Linking & Cloud Operations
