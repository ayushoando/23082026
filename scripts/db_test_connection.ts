...
export async function checkDatabaseConnection(
  ...
  const productsUrl = resolveProductsUrl();
  const plannerUrl = resolvePlannerUrl();
  if (
    env.CI === "true" &&
    !productsUrl?.trim() &&
    !plannerUrl?.trim()
    ...
    warn(
      "⚠️ CI: skipping database connection check (repository secrets not configured).",
    );
    return {
      ok: true,
      products: {
