...
async function main() {
  ...
  if (adminUrl && adminKey) {
    ...
    for (const table of ADMIN_TABLES) {
      ...
      try {
        const { rows, pages } = await fetchAllRows(adminClient, table, maxRetries);
        const file = `admin-${table}.json`;
        fs.writeFileSync(
          path.join(runDir, file),
          JSON.stringify(rows, null, 2),
          "utf8",
        );
        results.push({ table: `admin.${table}`, rows: rows.length, pages, status: "ok", file });
        console.log(`[backup] admin.${table}: ${rows.length} rows`);
      } catch (error) {
        const message = error instanceof Error ? error.message : String(error);
        if (isMissingTableError(message)) {
          results.push({
          ...
          console.warn(`[backup] admin.${table}: SKIPPED missing table`);
          continue;
        }
        results.push({
          table: `admin.${table}`,
        ...
        console.error(`[backup] admin.${table}: ERROR ${message}`);
      }
    }
  } else {
    console.warn("[backup] admin pass skipped — NEXT_ADMIN_SUPABASE_URL / SUPABASE_ADMIN_SERVICE_ROLE_KEY unset");
  }
