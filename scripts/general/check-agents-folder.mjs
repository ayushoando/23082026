...
if (!fs.existsSync(agentsDir) || !fs.statSync(agentsDir).isDirectory()) {
  ...
  for (const entry of entries) {
    if (entry.isDirectory()) {
      ...
      const reviewEntries = fs.readdirSync(path.join(agentsDir, "review"), {
        withFileTypes: true,
      });
      for (const nested of reviewEntries) {
        if (nested.isDirectory()) {
          violations.push(`extra directory under Agents/review/: ${nested.name}`);
        } else if (nested.isFile() && !nested.name.endsWith(".md")) {
          violations.push(`non-Markdown under Agents/review/: ${nested.name}`);
        }
      }
      continue;
    }
