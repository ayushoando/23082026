...
async function checkUrl(name: string, url: string) {
  try {
    ...
    if (url.includes("localhost")) {
      logError(
        name,
        new Error("Localhost not reachable (service may be down)"),
      );
    } else {
      logError(name, e);
    }
...
async function checkAdminSupabase() {
  ...
  if (!url || !adminKey)
    throw new Error("Missing admin Supabase URL or service role key");
...
async function checkCloudflareR2() {
  ...
  const url = `https://api.cloudflare.com/client/v4/accounts/${accountId}/r2/buckets/${bucket}/objects?list=true&limit=1`;
...
async function checkResendInbox() {
  ...
  if (!inbox)
    return logError("RESEND_INBOX", new Error("Missing RESEND_INBOX"));
  const emailRegex = /^[^@\s]+@[^@\s]+\.[^@\s]+$/;
  if (emailRegex.test(inbox)) {
    logSuccess("RESEND_INBOX", `valid email format (${inbox})`);
  } else {
    try {
...
async function checkCloudflareS3Url() {
  ...
  if (!url)
    return logError(
      "CLOUDFLARE_S3_URL",
      new Error("Missing CLOUDFLARE_S3_URL"),
    );
  try {
    const res = await fetch(url);
...
async function checkPostgres(name: string, connVar: string) {
  ...
  try {
    const rows = await sql<{ connected: number }[]>`SELECT 1 AS connected`;
    if (rows[0]?.connected === 1) logSuccess(name, "connection OK");
  } catch (e) {
    logError(name, e);
  } finally {
...
(async () => {
  try {
    ...
    if (process.env.NEXT_PUBLIC_SITE_URL)
      await checkUrl("Site URL", process.env.NEXT_PUBLIC_SITE_URL);
    if (process.env.NEXT_PUBLIC_TECH_DOCS_URL)
      await checkUrl("Tech Docs URL", process.env.NEXT_PUBLIC_TECH_DOCS_URL);
    if (process.env.SUPABASE_AUTH_URL)
      await checkUrl("Supabase Auth URL", process.env.SUPABASE_AUTH_URL);
    ...
    await Promise.all([
      ...
      checkPostgres("Products Postgres", "PRODUCTS_DATABASE_URL").catch((e) =>
        logError("Products Postgres", e),
      ),
      checkPostgres("Auth Postgres", "SUPABASE_AUTH_DATABASE_URL").catch((e) =>
        logError("Auth Postgres", e),
      ),
      checkPostgres("Planner Postgres", "PLANNER_DATABASE_URL").catch((e) =>
        logError("Planner Postgres", e),
      ),
    ]);
