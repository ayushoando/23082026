...
/** Read a raw env value from `.env.local` (no substitution, no export). */
...
/** Vercel token from env or `.env.local`. */
function vercelToken() {
  return process.env.VERCEL_TOKEN?.trim() || envLocalValue("VERCEL_TOKEN");
}

/** Products ref from `.env.local` `SUPABASE_URL` / `NEXT_PUBLIC_SUPABASE_URL`. */
function productsProjectIdFromEnv() {
  const envPath = path.join(ROOT, ".env.local");
  if (fs.existsSync(envPath)) {
    for (const line of fs.readFileSync(envPath, "utf8").split(/\r?\n/)) {
      const match = line.match(
        /^(?:NEXT_PUBLIC_)?SUPABASE_URL=(.+)$/,
      );
      if (!match) continue;
      ...
      try {
        ...
        // fall through to the documented Products project id
      }
    }
  }
...
const COMMANDS = {
  ...
  // Forward extra args (e.g. --dry). Always force admin target first so
  // `db:apply:admin -- --dry` works the same as `db:apply -- --target admin --dry`.
  "db:apply:admin": (args) =>
    runTsx("db_apply_migrations.ts", ["--target", "admin", ...args]),
  "db:test": () => runTsx("db_test_connection.ts"),
  "db:types": () => {
    const projectId = productsProjectIdFromEnv();
    ...
    process.stdout.write(
      `Wrote site/platform/types/database.types.ts from project ${projectId}\n`,
    );
  },
  ...
  "vercel:prod": () => {
    ...
    const token = vercelToken();
    const args = ["dlx", "vercel", "--prod", "--yes"];
    if (token) args.push("--token", token);
    run("pnpm", args);
  },
