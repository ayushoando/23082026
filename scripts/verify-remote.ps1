# Verify remote ayushoando/oando1408 has all files
...
$repo = gh repo view ayushoando/oando1408 --json name,isPrivate,url,defaultBranchRef,pushedAt 2>&1
L "=== Repo ==="
L ($repo | Out-String)

L "=== Remote commit ==="
L (gh api repos/ayushoando/oando1408/commits/main --jq '.sha, .commit.message' 2>&1 | Out-String)

L "=== Remote tree file count ==="
$tree = gh api 'repos/ayushoando/oando1408/git/trees/main?recursive=1' --jq '.truncated, (.tree | length)' 2>&1
L ($tree | Out-String)
