...
async function translateBatch(entries, locale, label) {
  ...
  const defaultOrigin = ["https:", "", "openrouter.ai", "api", "v1"].join("/");
  const defaultSite = ["https:", "", "oando.co.in"].join("/");
  const baseUrl = process.env.OPENROUTER_BASE_URL?.trim() || defaultOrigin;
  const siteUrl = process.env.NEXT_PUBLIC_SITE_URL?.trim() || defaultSite;
      ...
      try {
        ...
        if (!response.ok || data.error) {
          const retryAfter = Number(
            data.error?.metadata?.retry_after_seconds ?? 0,
          );
          if (retryAfter > 0) {
          ...
          throw new Error(
            `OpenRouter failed (${label}): ${JSON.stringify(data.error ?? data).slice(0, 400)}`,
          );
        }

        const content = data.choices?.[0]?.message?.content;
        const parsed = JSON.parse(
          content.trim().replace(/^```json\s*|\s*```$/g, ""),
        );
        return parsed;
      } catch (error) {
        lastError = error;
...
async function main() {
  ...
  for (const locale of manifest.deferredLocales) {
    ...
    const merged = fs.existsSync(file) ? readJson(file) : {};
    ...
    for (const namespace of manifest.allMarketingNamespaces) {
      ...
      for (const leaf of leaves) {
        const current = collectLeaves(merged[namespace]).find(
      ...
      if (pending.length === 0) {
        ...
        merged[namespace] = deepMergeStructure(
          en[namespace],
          merged[namespace] ?? {},
        );
        continue;
      ...
      process.stdout.write(
        `Translating ${locale}/${namespace} (${pending.length} strings)\n`,
      );
