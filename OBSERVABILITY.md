...
- **2026-09-07 site-only Vercel environment reset:** Existing variables were removed from the linked `23082026` project, then 53 nonblank names from `site/.env.local` matching `site/.env.example` were recreated in **Production only**. Preview contains 0 variables. The sync helper now reads no root environment file and excludes `DEV_AUTH_BYPASS` plus local-only test/operator credentials. Values were not printed or committed; no deployment or New Relic account/dashboard mutation was performed in this step.
