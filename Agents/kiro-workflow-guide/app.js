(() => {
  ...
  $('#source-summary').textContent = `${data.sources.length} sources · ${data.powers.length} powers · ${Object.values(data.opsGroups).flat().length} ops`;
  $('#source-count').textContent = `${data.sources.length} source entries mapped · ${data.skills.length} skills · ${data.powers.length} powers`;
