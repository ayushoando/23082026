(() => {
  ...
  const usageCopy = (name) => {
    const usage = data.usage?.[name];
    if (!usage) return '';
    return `<div class="usage-block"><p><strong>Use when:</strong> ${escapeHtml(usage.when)}</p><p><strong>Do this:</strong> ${escapeHtml(usage.action)}</p><p class="usage-avoid"><strong>Avoid:</strong> ${escapeHtml(usage.avoid)}</p></div>`;
  };

  $('#route-grid').innerHTML = data.routes.map((item) => `<article class="route-card searchable" data-search="${escapeHtml(item.search)}"><div class="route-number ${item.tone}">${item.icon}</div><h3>${escapeHtml(item.title)}</h3><p>${escapeHtml(item.text)}</p><code>${escapeHtml(item.action)}</code></article>`).join('');
  ...
  $('#skills-grid').innerHTML = data.skills.map((item) => `<article class="info-card searchable" data-search="${escapeHtml(item.search)}"><div class="card-icon ${item.tone}">${item.icon}</div><div class="card-meta">${item.meta}</div><h3>${item.name}</h3>${usageCopy(item.name)}<p>${item.text}</p><code>${item.command}</code></article>`).join('');
  ...
  $('#powers-grid').innerHTML = data.powers.map((item) => `<article class="power-row searchable" data-search="${escapeHtml(item.search)}"><div class="power-symbol ${item.tone}">${item.symbol}</div><div class="power-body"><div class="card-meta">${item.meta}</div><h3>${item.name}</h3>${usageCopy(item.name)}<p>${item.text}</p><div class="tag-row">${tags(item.tags)}</div></div></article>`).join('');
